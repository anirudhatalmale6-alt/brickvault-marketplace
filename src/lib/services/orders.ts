...
export async function createOrder(
  ...
  const { data: cartItems } = await sb
    .from("cart_items")
    .select("*, listings(id, price, seller_id, status)")
    .in("id", input.cartItemIds)
    .eq("user_id", user.id);

  if (!cartItems?.length) return { data: null, error: "No valid cart items" };

  const active = cartItems.filter((i) => i.listings?.status === "active");
  if (!active.length) return { data: null, error: "No active listings in cart" };

  const total = active.reduce((s, i) => s + Number(i.listings?.price ?? 0) * i.quantity, 0);
  const sellerId = active[0].listings?.seller_id;
